...
\begin{flushleft}
% Adjust height as needed; replace path if logo file differs
\includegraphics[height=1.8cm]{media/giu.png}% University Logo Placeholder
\end{flushleft}
\vspace{-1.2em} % tighten space before title
...
The \textbf{workpiece} is the \emph{passenger passage aperture} governed by two coupled sliding leaves.
	extbf{Preliminary dimensional assumptions (validate in Milestone 2):}
\begin{itemize}[noitemsep]
...
\begin{enumerate}[label=Step \arabic*:,leftmargin=2.2cm]
	...
	\item \textbf{Opening}: Command Open. DC Extends driving racks; MSC Extends simultaneously. IR beam monitored but ignored for opening. Reed Open switch triggers at end-of-stroke.
...
% =============================================================
\subsection*{Mechanical Actuation Mechanism (Updated)}
...
\end{tikzpicture}
\caption{Pneumatic position diagram ("A+/A-/B+/B-/C+/C-"): valve coil energization across steps.}
\end{figure}

	extbf{NOTE:} The time diagram above mirrors the tabular sequence. An obstruction branch can be shown as a second overlay (OB: immediate return to A+/B+ high and inhibit C-).
